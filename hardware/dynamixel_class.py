...
class DynamixelInterface:
    ...
    def set_moving_speed(self, dxl_id, speed=0, torque=500):
        # check 0 <= torque <= 1023
    ...
    def forward(self, speed=500, torque=1023):
            interface.set_moving_speed(1, speed, torque)
            interface.set_moving_speed(2, 1024+speed, torque)
            interface.set_moving_speed(3, 1024+speed, torque)
            interface.set_moving_speed(4, speed, torque)
    
    def right(self, speed=500, torque=1023):
            interface.set_moving_speed(1, speed, torque)
            interface.set_moving_speed(2, speed, torque)
            interface.set_moving_speed(3, 1024+speed, torque)
            interface.set_moving_speed(4, 1024+speed, torque)
    
    def backward(self, speed=500, torque=1023):
            interface.set_moving_speed(1, 1024+speed, torque)
            interface.set_moving_speed(2, speed, torque)
            interface.set_moving_speed(3, speed, torque)
            interface.set_moving_speed(4, 1024+speed, torque)
    
    def left(self, speed=500, torque=1023):
            interface.set_moving_speed(1, 1024+speed, torque)
            interface.set_moving_speed(2, 1024+speed, torque)
            interface.set_moving_speed(3, speed, torque)
            interface.set_moving_speed(4, speed, torque)
    
    def drive45(self, speed=500, torque=1023):
            interface.set_moving_speed(1, self.tf_speed(speed*1), torque)
            interface.set_moving_speed(2, self.tf_speed(0), torque)
            interface.set_moving_speed(3, self.tf_speed(-speed*1), torque)
            interface.set_moving_speed(4, self.tf_speed(), torque)
            
    def drive(self, angle=45, speed=500, torque=1023): 
            speeds_trig = numpy.array( [+cos(radians(angle) - radians(45)), +cos(radians(angle) - radians(135)), -cos(radians(angle) - radians(45)), -cos(radians(angle) - radians(135))]) * speed
            speeds = [self.tf_speed(i) for i in speeds_trig]
            # print(speeds_trig)
            print(speeds)
            interface.set_moving_speed(1, speeds[0], torque)
            interface.set_moving_speed(2, speeds[1], torque)
            interface.set_moving_speed(3, speeds[2], torque)
            interface.set_moving_speed(4, speeds[3], torque)
    
    def anticlockwise(self, speed=500, torque=1023):
            interface.set_moving_speed(1, 1024+speed, torque)
            interface.set_moving_speed(2, 1024+speed, torque)
            interface.set_moving_speed(3, 1024+speed, torque)
            interface.set_moving_speed(4, 1024+speed, torque)
    ...
    def clockwise(self, speed=500, torque=1023):
            interface.set_moving_speed(1, speed, torque)
            interface.set_moving_speed(2, speed, torque)
            interface.set_moving_speed(3, speed, torque)
            interface.set_moving_speed(4, speed, torque)
    ...
    def circular_clockwise(self, speed=500, torque=1023):
        for i in range(180):
            interface.drive(i*4)
            time.sleep(0.01)
...
if __name__ == "__main__":
    try:
        ...
        # interface.testing_motors_moving_speed()
        # Forward
        # (2) +1 /  \ -1 (3)
        # (1) +1 \  / -1 (4)
        # while 1:
        #     x = int(input("input:speed"))
        #     print(interface.set_moving_speed(2, speed=x, torque=500)) 
        while 1:
            interface.circular_clockwise(200, 200)
            # interface.drive(45, 200)
            # interface.forward()
            time.sleep(1)
            # interface.right()
            # time.sleep(1)
