...
class PathFollowing(State):
    ...
    def execute(self, blackboard: Blackboard) -> str:
        ...
        while True:
            # delta_x = communicator.object_coords[0] - rob_pose.x
            # delta_y = communicator.object_coords[1] - rob_pose.y
            # time_x  = blackboard["object_info"][0] * 4.2
            # yasmin.YASMIN_LOG_INFO(f"{time_x=}s")
            # time_x  = delta_x * 5
            # time_y  = delta_y * 5
            
            
            # THETA-movement 45 degree
            ...
            while (abs(45 - cam_odom.current_position["theta"])) > 2: # err less than 2 degree
                print(f'{cam_odom.current_position=}')
                self.ros_manager.publish_cmd_vel(twist_msg)
            self.ros_manager.publish_cmd_vel(Twist()) # STOP MSG
            _x = cam_odom.current_position["theta"]
            print(f"arrvied at 45 degree @ {_x}")
            time.sleep(3)
            
            def globaltorobottf(global_x_vel, global_y_vel):
                sin_cos_45 = 0.70710678118
                out_x = (-global_x_vel*sin_cos_45 + global_y_vel*sin_cos_45 ) * 9
                out_y = (global_x_vel*sin_cos_45 + global_y_vel*sin_cos_45 ) * 9
                return out_x, out_y
            ...
            x = 0.5
            while True:
                err_x = x - cam_odom.current_position["x"]
                print(f'{err_x=}')
                print(f'{cam_odom.current_position=}')
                print(f'{twist_msg=}')
                if (abs(err_x)) < 0.03: # err less than 0.05m
                    break
                twist_msg.linear.x, twist_msg.linear.y =  globaltorobottf(err_x, 0)
                self.ros_manager.publish_cmd_vel(twist_msg)
                time.sleep(0.01)
            self.ros_manager.publish_cmd_vel(Twist()) # STOP MSG
            print(f"arrvied at x= {x}")
            time.sleep(2)
            
            twist_msg = Twist()
            y = 0.5
            while True:
                err_y = y - cam_odom.current_position["y"]
                print(f'{err_y=}')
                print(f'{cam_odom.current_position=}')
                print(f'{twist_msg=}')
                if (abs(err_y)) < 0.03: # err less than 0.05m
                    break
                twist_msg.linear.x, twist_msg.linear.y =  globaltorobottf(0, err_y)
                self.ros_manager.publish_cmd_vel(twist_msg)
                time.sleep(0.01)
            self.ros_manager.publish_cmd_vel(Twist()) # STOP MSG
            print(f"arrvied at y= {y}")
            time.sleep(2)
        ...
        """
        if fake:
            yasmin.YASMIN_LOG_INFO(bcolors.YELLOW_WARNING + f"Executing state FakePathFollowing" + bcolors.ENDC)
            communicator.object_coords # x, y, theta
            rob_pose = self.ros_manager.get_latest_pose()
            while True:
                delta_x = communicator.object_coords[0] - rob_pose.x
                ...
                while (time.time() - start_time) < time_x:
                    self.ros_manager.publish_cmd_vel(twist_msg)
                ...
                # twist_msg.linear.y = 0.5
                # start_time = time.time()
                # yasmin.YASMIN_LOG_INFO(bcolors.BLUE_OK + f"Moving in Y-Direction" + bcolors.ENDC)
                # while (time.time() - start_time) < time_y:
                #     self.ros_manager.publish_cmd_vel(twist_msg)
                # self.ros_manager.publish_cmd_vel(Twist()) # STOP MSG
                break
        else:
            ...
            for path in paths:
                ...
                while True:
                    err_pose_x = self.ros_manager.get_latest_pose().x - path[0]
                    err_pose_y = self.ros_manager.get_latest_pose().y - path[1]
                    if (err_pose_x**2 + err_pose_y**2) > 0.005:
                        t = time.time_ns()
                        delta_t = (t-prev_t)*10**9
                        prev_t = int(t)
                        sum_e_x += err_pose_x * delta_t
                        sum_e_y += err_pose_y * delta_t
                        de_dt_x = (err_pose_x - prev_err_pose[0])/delta_t
                        de_dt_y = (err_pose_y - prev_err_pose[1])/delta_t
                        
                        vel = [
                            self.linear_kp * err_pose_x + self.linear_ki * sum_e_x + self.linear_kd * de_dt_x,
                            self.linear_kp * err_pose_y + self.linear_ki * sum_e_y + self.linear_kd * de_dt_y
                        ]
                        [twist_msg.linear.x, twist_msg.linear.y] = vel
                        yasmin.YASMIN_LOG_INFO(f"err_pose_x {err_pose_x} err_pose_y {err_pose_y}\n\ttwist_pose {vel}")
                        self.ros_manager.publish_cmd_vel(twist_msg)
                    else:
                        break
