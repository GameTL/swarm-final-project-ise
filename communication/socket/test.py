...
class Communicator:
    ...
    def handle_connection(self, client_fd):
        try:
            ...
            if synack == "SYNACK":
                print("[INFO] SYNACK received. Communication complete.")
    ...
    def consensus(self, new_claimant):
        if new_claimant == self.identifier:
            print(f"[INFO] ({self.identifier}) am claiming taskmaster.")
        ...
        if len(unique_claims) == 1:
            ...
            if vote_count[most_voted] > len(self.taskmaster_claims) // 2:
                final_taskmaster = most_voted

        # Update priority queue (push taskmaster to back)
        if final_taskmaster in self.priority_queue:
            self.priority_queue.remove(final_taskmaster)
            self.priority_queue.append(final_taskmaster)

        print(f"[INFO] Consensus reached: {final_taskmaster} is the taskmaster.")
        
        # Broadcast consensus to all robots
        self.broadcast("CONSENSUS_REACHED", final_taskmaster)
