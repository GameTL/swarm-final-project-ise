...
class FormationMaster:
    def __init__(self, current_coords: dict, object_coords: tuple, radius=0.3, verbose=False):
        """ 
    ...
    def plan_paths(self):
        ...
        for i, (robot_id, target) in enumerate(self.target_coords):
            print(f"Assigning {robot_id}'s path: {self.current_coords_dict[robot_id][0:2:1]} -> {target}")
            # print(f'{self.current_coords[i][1]=}')
            print(f'{(self.current_coords_dict[robot_id][0:2:1], target)=}')
            # path = self.path_planning_algorithm(self.current_coords_dict[robot_id][0:2:1], target, verbose=self.verbose)
...
if __name__ == "__main__":
    ...
    current_positions = [("TurtleBot3Burger_1",(0, 5)),("TurtleBot3Burger_2", (1, 4)),("TurtleBot3Burger_3", (0, 3))]
